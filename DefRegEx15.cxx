...
int main( int argc, char *argv[] )
{
  ...
  typedef itk::MattesMutualInformationImageToImageMetric<
                                    FixedImageType,
									MovingImageType >    MetricTypeMattes;
  ...
  MetricTypeMattes::Pointer         Matesmetric = MetricTypeMattes::New();
  //MetricType::Pointer         metric = MetricType::New();
  GradientOptimizerType::Pointer      gradientOptimizer = GradientOptimizerType::New();
  LBFGOptimizerType::Pointer      optimizer = LBFGOptimizerType::New();
  ...
  registration->SetMetric(Matesmetric);
 ...
 registration->SetFixedImageRegion( fixedRegion );
  ...
  std::cout << "CCCCCCCCCCCCCCCCCCC   Rigid Transform nr of Params: " << rigidTransform->GetNumberOfParameters() << std::endl;
  ...
  //
  typedef GradientOptimizerType::ScalesType       OptimizerScalesType;
  ...
  gradientOptimizer->SetScales(optimizerScales);

  gradientOptimizer->SetMaximumStepLength(0.05000);
  gradientOptimizer->SetMinimumStepLength(0.0001);

  gradientOptimizer->SetNumberOfIterations(500);
  ...
  //Matesmetric->SetNumberOfSpatialSamples(10000L);
  Matesmetric->SetNumberOfSpatialSamples(numberOfSamples / 10);

  //
  // Create the Command observer and register it with the optimizer.
  //
  CommandIterationUpdate::Pointer observerGrad = CommandIterationUpdate::New();
  gradientOptimizer->AddObserver(itk::IterationEvent(), observerGrad);
  ...
  {
	  std::cerr << "ExceptionObject caught dupa!" << std::endl;
	  std::cerr << err << std::endl;
	  return EXIT_FAILURE;
  }
  ...
  if (argc >= 6)
  {
	  try{
		  ...
		  std::cerr << "Exception while writing transform parameter file" << std::endl;
  ...
  try
  {
	  std::cout << "Writing RIGID image ...";
	  writer->Update();
	  //update
  }
  catch (itk::ExceptionObject & err)
  {
	  std::cerr << "ExceptionObject caught !------3" << std::endl;
	  std::cerr << err << std::endl;
	  return -1;
  }
  ...
  if (argc >= 5)
  {
	  ...
	  {
		  writer2->Update();
		  std::cout << "Difference RIGID completed" << std::endl;
		  //update
	  }
	  catch (itk::ExceptionObject & err)
	  {
		  std::cerr << "ExceptionObject caught !------4" << std::endl;
		  std::cerr << err << std::endl;
		  return -1;
	  }
  }
  ...
  // multi-resolution registration because it is indeed a sub-sampling of the
  // image.
  MetricTypeMattes->SetNumberOfSpatialSamples(numberOfBSplineParameters * 100);
  ...
  MetricTypeMattes->SetNumberOfSpatialSamples(numberOfSamples);
